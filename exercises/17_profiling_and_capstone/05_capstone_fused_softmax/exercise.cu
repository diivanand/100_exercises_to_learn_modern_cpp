// =============================================================================
//  17.05 -- Capstone: a fused softmax
// =============================================================================
//
//  The last exercise of the CUDA track. Softmax turns a row of scores into a
//  row of probabilities -- exp(x_i) / sum_j exp(x_j) -- and every attention
//  head in every transformer computes it billions of times (Cautaerts ch. 14
//  builds the transformer that calls it; the PyTorch course's capstone is the
//  same model from the other side). It is small enough to fit in one file
//  and rich enough to need everything the track has covered:
//
//   * ONE BLOCK PER ROW, with threads striding across the columns, so that
//     every global access is coalesced (15.01) and no two blocks ever share
//     an address.
//
//   * A BLOCK REDUCTION built from warp shuffles and one shared slot per
//     warp (15.03), used twice in a row. The second __syncthreads at the end
//     of it is the kind of detail racecheck (17.04) exists to catch when it
//     is missing.
//
//   * NUMERICAL STABILITY. exp overflows single precision at x = 89 (PMPP
//     §7.2). Subtracting the row maximum first changes nothing
//     mathematically and everything in practice. The third test feeds
//     logits in the hundreds.
//
//   * KERNEL FUSION. The starter does the job in two launches with the
//     row sums accumulated by atomicAdd: every thread of a row serialised
//     on one address (15.07), the statistics written to global memory and
//     read back, a memset and an allocation per call. Fusing it into a
//     single kernel removes all of that. The last test times both.
//
//   * RAII buffers (14.03), events and best-of timing (17.01) -- and when
//     it passes, `./mcpp profile 17_05` and `./mcpp profile 17_05 --ncu`
//     to see it: one kernel where there were two, memory throughput near
//     the roofline where there was atomic contention.
//
//  TASK
//    Implement `block_reduce`, then replace the two kernels with one fused,
//    stable softmax kernel behind the same `softmax_rows` signature.
//
//  RUN IT
//    ./mcpp test 17_05
//
...
// TODO: reduce across the whole block and return the result to EVERY thread.
//
// The shape (15.03): each warp reduces with shuffles; lane 0 of each warp
// writes its value to `scratch[warp]`; __syncthreads; warp 0 loads the
// per-warp values (identity for lanes beyond the last warp) and reduces
// them; thread 0 writes the answer to scratch[0]; __syncthreads; everyone
// reads it. Then one more __syncthreads, so the caller may reuse `scratch`
// for the next reduction without racing the slow readers of this one.
template <typename Op>
__device__ float block_reduce(float v, float identity, Op op, float* scratch) {
  (void)identity;
  (void)scratch;
  return warp_reduce(v, op);
}

// TODO: replace the two kernels below with ONE kernel, one block per row:
//
//   1. each thread takes the max over its columns (c = threadIdx.x, then
//      c += kThreads: consecutive threads read consecutive columns, so the
//      reads coalesce, 15.01), then block_reduce(..., MaxOp{}) gives the
//      row maximum to every thread;
//   2. the same for the sum of expf(x - max);
//   3. write expf(x - max) / sum.
//
// Subtracting the row maximum is not an optimisation. expf(89) is already
// inf in single precision (PMPP §7.2, "Representable Numbers"), and
// inf / inf is NaN. Every softmax you have ever used does this; the third
// test is what happens when one does not.
//
// The reads of the row in steps 2 and 3 come from cache: a 1024-float row
// is 4 KB, and there is one block per row. What the fusion saves is the
// global writes and re-reads of the statistics, two launches, the memset,
// the allocation, and several thousand threads serialised on one address
// per row by atomicAdd.

__global__ void row_sum_exp_kernel(const float* in, float* sums, int cols) {
  const int row = static_cast<int>(blockIdx.x);
  for (int c = static_cast<int>(threadIdx.x); c < cols; c += kThreads) {
    atomicAdd(&sums[row], expf(in[static_cast<std::size_t>(row) * cols + c]));
  }
}

__global__ void normalise_kernel(const float* in, const float* sums, float* out,
                                 int cols) {
  const int row = static_cast<int>(blockIdx.x);
  for (int c = static_cast<int>(threadIdx.x); c < cols; c += kThreads) {
    const std::size_t i = static_cast<std::size_t>(row) * cols + c;
    out[i] = expf(in[i]) / sums[row];
  }
}
...
void softmax_rows(const float* in, float* out, int rows, int cols) {
  ...
  row_sum_exp_kernel<<<grid, kThreads>>>(in, sums.data(), cols);
  CUDA_CHECK(cudaGetLastError());
  normalise_kernel<<<grid, kThreads>>>(in, sums.data(), out, cols);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
}
