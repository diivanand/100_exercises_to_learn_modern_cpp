// =============================================================================
//  15.01 -- Memory coalescing
// =============================================================================
//
//  A GPU does not read one float at a time. When the 32 threads of a warp
//  execute a load, the hardware bundles their addresses into memory
//  transactions of fixed size and alignment: 32-byte sectors on every GPU
//  since Volta (Cautaerts ch. 5, "Efficient global memory access patterns").
//  If the 32 threads ask for 32 consecutive floats, that is 128 contiguous
//  bytes, four sectors, and every byte fetched is used. The access is
//  COALESCED. If instead each thread reads a float 16 KB from its neighbour's
//  -- walking down a column of a row-major matrix -- the same instruction
//  touches 32 different sectors and uses 4 bytes of each: 1/8 of the
//  bandwidth does useful work.
//
//  PMPP §6.2 ("Global Memory Bandwidth") gives the rule that follows from
//  this: make threadIdx.x index the dimension along which consecutive elements
//  are adjacent in memory. For a row-major matrix that is the column index.
//  The rule has not changed since 2010; only the numbers have. G80 needed
//  accesses to be both consecutive AND aligned to 64 or 128 bytes; from Fermi
//  on the L1 and L2 caches take the edge off misalignment (PMPP §12.2.5
//  predicts exactly this), and on Ada an RTX 4090 moves about 1 TB/s from
//  DRAM through a 72 MB L2. But a strided pattern still costs 8x, because the
//  cache cannot invent locality that the code does not have.
//
//  The shape of the block matters too. `dim3 block(32, 8)` puts a whole warp
//  along x, so one warp covers 32 consecutive columns of one row. `dim3
//  block(8, 32)` would spread a warp over 8 columns of 4 rows: four separate
//  128-byte segments per instruction instead of one.
//
//  TASK
//    `scale_matrix` produces the right answer and runs 8x slower than it
//    should, because it maps threadIdx.x to the ROW index. Swap the mapping
//    in the kernel and in `launch_scale` so that a warp walks along a row.
//
//  RUN IT
//    ./mcpp test 15_01
//
...
// TODO: threadIdx.x indexes the row here. Thread 0 of a warp reads
// in[0 * width], thread 1 reads in[1 * width], ... -- 32 addresses each
// `width` floats apart, so one load instruction touches 32 memory sectors and
// uses 4 bytes of each. Make threadIdx.x index the COLUMN instead, so that a
// warp reads 32 adjacent floats.
...
// TODO: once the kernel maps x to columns, the grid must too: x should cover
// `width` and y should cover `height`. A grid that does not match its kernel
// is the second most common CUDA bug after a missing bounds guard.
void launch_scale(const float* in, float* out, int width, int height, float factor) {
  const dim3 block(kBlockX, kBlockY);
  const dim3 grid((static_cast<unsigned>(height) + kBlockX - 1) / kBlockX,
                  (static_cast<unsigned>(width) + kBlockY - 1) / kBlockY);
