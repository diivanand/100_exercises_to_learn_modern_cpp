// =============================================================================
//  16.07 -- Device lambdas and templates
// =============================================================================
//
//  Everything chapter 08 and chapter 09 taught applies to device code, with
//  one extra word. A kernel can be a template (`map_kernel<T, F>`), it can be
//  instantiated for any element type and any callable, and `if constexpr`
//  discards the branch that does not apply to this T -- so one `fast_exp`
//  gives float the `__expf` intrinsic and double the accurate `exp`, with no
//  overload set and no runtime test. The compiler knows the types; it makes
//  the decision.
//
//  The extra word is `__device__`. A function called from a kernel must be
//  compiled for the device, and a lambda's call operator is a function:
//
//      [factor] __device__ (float x) { return x * factor; }
//
//  This is an "extended lambda", an nvcc extension enabled by
//  `--extended-lambda` (cmake/Cuda.cmake sets it). Without the marking, the
//  closure is host-only and the kernel that calls it is a compile error --
//  the same error 14.05 was about, now one level of indirection away. The
//  restrictions worth knowing: an extended lambda must be defined inside a
//  named function (not at namespace scope, not in a function with a deduced
//  return type), it captures by value only, and `__host__ __device__` makes
//  it callable on both sides, which is what Thrust wants (16.01). See the
//  CUDA C++ Programming Guide, "Extended Lambdas".
//
//  A functor -- a struct with a `__device__ operator()` -- has no such
//  restrictions and is what the CUB and Thrust headers themselves use; the
//  tests here use one. Lambdas are for the call site that would otherwise
//  need a one-off struct.
//
//  CONSTRAIN AT THE BOUNDARY. `map` is the host function callers see, so it
//  is the place for the C++20 constraint (`std::floating_point T`, 08.07):
//  a caller who passes integers gets one line naming the requirement, not a
//  page of failures from inside a kernel instantiation. The kernel template
//  itself stays unconstrained; it is an implementation detail.
//
//  TASK
//    Mark the lambda in `scale_all` as device code, and constrain `map` to
//    floating-point element types.
//
//  NOTE  This exercise starts as a compile error, twice over: the unmarked
//        lambda cannot be called from the kernel, and a static_assert in the
//        tests checks that `map` rejects integer elements.
//
//  RUN IT
//    ./mcpp test 16_07
//
...
// TODO: constrain T to floating-point types (`std::floating_point T`, from
// <concepts>). Right now `map` accepts a std::vector<int> and the failure, if
// any, surfaces deep inside the kernel instantiation.
template <typename T, typename F>
...
// TODO: this lambda is host code, and the kernel cannot call it. Mark it
// `__device__` (the marking goes between the capture list and the parameter
// list).
std::vector<float> scale_all(const std::vector<float>& host, float factor) {
  return map(host, [factor](float x) { return x * factor; });
}
