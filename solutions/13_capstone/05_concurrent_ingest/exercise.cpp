...
public:
  IngestPipeline(ConcurrentStore& store, int worker_count) : store_(store) {
    ...
    for (int i = 0; i < worker_count; ++i) {
      // An exception escaping a thread's callable calls std::terminate, so a
      // real ingest worker would catch, log and continue here. The store's
      // operations can still throw std::bad_alloc, which is the one failure
      // nothing at this level can do anything useful about.
