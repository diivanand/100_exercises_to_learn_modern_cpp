...
public:
  Pipeline(const DeviceBuffer<float>& x, const DeviceBuffer<float>& y,
    ...
    // Between BeginCapture and EndCapture nothing runs: each launch on the
    // capturing stream becomes a node, and the stream order becomes the
    // dependency edges. Every launch must name the stream -- a launch on the
    // default stream is not captured (and, in global mode, is an error).
    CUDA_CHECK(cudaStreamBeginCapture(stream_, cudaStreamCaptureModeGlobal));
    scale_kernel<<<grid, kBlock, 0, stream_>>>(x.data(), factor, tmp_.data(), n);
    cudaError_t launch_status = cudaGetLastError();
    add_kernel<<<grid, kBlock, 0, stream_>>>(tmp_.data(), y.data(), out.data(), n);
    if (launch_status == cudaSuccess) {
      launch_status = cudaGetLastError();
    }
    square_kernel<<<grid, kBlock, 0, stream_>>>(out.data(), n);
    if (launch_status == cudaSuccess) {
      launch_status = cudaGetLastError();
    }
