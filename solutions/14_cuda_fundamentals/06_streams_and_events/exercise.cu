// Solution -- 14.06 Streams and events
...
void enqueue_pipeline(const Streams& streams, const DeviceBuffer<float>& d_in,
  ...
  // Different streams have no ordering between them. The event records the
  // point in the compute stream after the kernel, and the copy stream is told
  // to wait for it before running anything enqueued after the wait.
  CUDA_CHECK(cudaEventRecord(streams.kernel_done(), streams.compute()));
  CUDA_CHECK(cudaStreamWaitEvent(streams.copy(), streams.kernel_done(), 0));
